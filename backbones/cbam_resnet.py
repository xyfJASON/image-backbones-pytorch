...
class ChannelAttention(nn.Module):
    def __init__(self, C: int, r: int):
        ...
        self.mlp = nn.Sequential(
            nn.Conv2d(C, C // r, kernel_size=(1, 1)),
            nn.ReLU(inplace=True),
...
class CBAMResNet(nn.Module):
    def __init__(self, block_type: str, n_blocks: List[int], first_block: nn.Module, n_classes: int):
        ...
        if block_type == 'basic':
            self.conv2_x = self._make_layer(CBAMBasicBlock, n_blocks[0], [64, 64], reduce=False)
            self.conv3_x = self._make_layer(CBAMBasicBlock, n_blocks[1], [64, 128], reduce=True)
            self.conv4_x = self._make_layer(CBAMBasicBlock, n_blocks[2], [128, 256], reduce=True)
            self.conv5_x = self._make_layer(CBAMBasicBlock, n_blocks[3], [256, 512], reduce=True)
        else:
            self.conv2_x = self._make_layer(CBAMBottleneckBlock, n_blocks[0], [64, 64, 256], reduce=False)
            self.conv3_x = self._make_layer(CBAMBottleneckBlock, n_blocks[1], [256, 128, 512], reduce=True)
            self.conv4_x = self._make_layer(CBAMBottleneckBlock, n_blocks[2], [512, 256, 1024], reduce=True)
            self.conv5_x = self._make_layer(CBAMBottleneckBlock, n_blocks[3], [1024, 512, 2048], reduce=True)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
    ...
    @staticmethod
    def _make_layer(CBAMResidualBlock, n_block: int, channels: List[int], reduce: bool):
        layers = []
        for _ in range(n_block):
            layers.append(CBAMResidualBlock(*channels, reduce=reduce))
...
def cbam_resnet18(n_classes, first_block: str = 'cifar10'):
    ...
    model = CBAMResNet('basic', [2, 2, 2, 2], first_block, n_classes=n_classes)
    return model
...
def cbam_resnet34(n_classes, first_block: str = 'cifar10'):
    assert first_block in ['cifar10', 'imagenet']
    first_block = cifar10_first_block() if first_block == 'cifar10' else imagenet_first_block()
    model = CBAMResNet('basic', [3, 4, 6, 3], first_block, n_classes=n_classes)
    return model
...
def cbam_resnet50(n_classes, first_block: str = 'cifar10'):
    assert first_block in ['cifar10', 'imagenet']
    first_block = cifar10_first_block() if first_block == 'cifar10' else imagenet_first_block()
    model = CBAMResNet('bottleneck', [3, 4, 6, 3], first_block, n_classes=n_classes)
    return model
...
def cbam_resnet101(n_classes, first_block: str = 'cifar10'):
    assert first_block in ['cifar10', 'imagenet']
    first_block = cifar10_first_block() if first_block == 'cifar10' else imagenet_first_block()
    model = CBAMResNet('bottleneck', [3, 4, 23, 3], first_block, n_classes=n_classes)
    return model
...
def cbam_resnet152(n_classes, first_block: str = 'cifar10'):
    assert first_block in ['cifar10', 'imagenet']
    first_block = cifar10_first_block() if first_block == 'cifar10' else imagenet_first_block()
    model = CBAMResNet('bottleneck', [3, 8, 36, 3], first_block, n_classes=n_classes)
    return model
...
def _test_overhead():
    ...
    model = cbam_resnet18(n_classes=10).to(device)
